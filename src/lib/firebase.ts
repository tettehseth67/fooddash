...
async function testConnection() {
  try {
    ...
    console.error("Firebase Connection Result:", err.code, err.message);
    
    if (err.code === 'unavailable' || (err.message && err.message.includes('the client is offline'))) {
      ...
      console.log("Firebase connection successful (reached server, but read was denied as expected)");
    }
  }
}
